...
export function Communications() {
  ...
  return (
    <PageLayout fullHeight headerActions={headerActions} contentClassName="gap-0 pb-3.5">
      <section className="grid min-h-0 flex-1 overflow-hidden rounded-md border border-border bg-card lg:grid-cols-[190px_330px_minmax(0,1fr)] 2xl:grid-cols-[190px_350px_minmax(0,1fr)_280px]">
            ...
            {CHANNELS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-md px-2.5 py-2 text-left text-[13px] transition-colors ${active ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-background hover:text-foreground"}`}
                >
        ...
        <aside className="flex min-h-0 flex-col border-r border-border/80">
          <div className="space-y-2.5 border-b border-border/80 p-3">
            ...
            {isLoading ? (
              ...
              <div className="p-2">
                {visibleConversations.map((conversation) => {
                  ...
                  return (
                    <button key={conversation.id} type="button" onClick={() => selectConversation(conversation)} className={`mb-1 w-full rounded-md p-2.5 text-left transition-colors ${active ? "bg-primary-light ring-1 ring-primary/20" : "hover:bg-muted/70"}`}>
        ...
        <main className="flex min-h-0 min-w-0 flex-col">
          {selected ? (
            <>
              <div className="flex min-h-14 items-center justify-between gap-3 border-b border-border/80 px-3 py-2.5 sm:px-4">
              ...
              <ScrollArea className="min-h-0 flex-1 bg-muted/10">
                <div className="mx-auto flex w-full max-w-4xl flex-col gap-2.5 p-3 sm:p-4">
                  {messagesLoading ? <div className="flex h-40 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div> : messages.length === 0 ? <div className="flex h-48 flex-col items-center justify-center text-center"><MessageSquare className="mb-3 h-8 w-8 text-muted-foreground/50" /><p className="text-sm font-medium"><T k="support.no_messages" /></p><p className="mt-1 text-xs text-muted-foreground"><T k="literal.communications.send_the_first_message_to_start_this_convers.2e1f2908" /></p></div> : messages.map((message) => {
                    ...
                    return <div key={message.id} className={`flex ${outgoing ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] rounded-md px-3 py-2 text-[13px] ${outgoing ? "rounded-br-md bg-primary text-primary-foreground" : "rounded-bl-md border border-border bg-card text-foreground"}`}><p className="whitespace-pre-wrap leading-5">{message.content}</p>{message.attachment_url && <a href={message.attachment_url} target="_blank" rel="noreferrer" className="mt-2 block text-xs underline"><T k="literal.communications.open_attachment.b36bd6b7" /></a>}<p className={`mt-1 text-[10px] ${outgoing ? "text-primary-foreground/70" : "text-muted-foreground"}`}>{new Date(message.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p></div></div>;
                  })}
                </div>
              </ScrollArea>

              <div className="border-t border-border/80 bg-card p-2.5 sm:p-3">
