...
const nextPrimary:Partial<Record<JobStatus,{status:JobStatus;label:string}>>={
  ...
  on_the_way:{status:"in_progress",label:"Start service"},
...
export function JobExecutionPage(){
  ...
  async function transition(next:JobStatus){if(!job||!canTransitionJobStatus(job.status,next))return;setSaving(true);setError(null);try{setJob(await updateJobStatus(repository,job.id,next))}catch(cause){setError(cause instanceof Error?cause.message:"Unable to update scheduled service")}finally{setSaving(false)}}
