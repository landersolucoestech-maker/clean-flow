...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-cleanflow-tenant",
...
const SERVICE_TYPES = [
  "Regular Cleaning", "Deep Cleaning", "First Cleaning", "One-Time Cleaning",
  "Move-In / Move-Out Cleaning", "Office Cleaning", "Commercial Cleaning",
  "Post-Construction Cleaning", "Cleaning for a Reason",
] as const;

const FREQUENCY_OPTIONS = [
  "Daily", "Weekly", "Regular Cleaning 2 Weeks", "Regular Cleaning 3 Weeks",
  "Regular Cleaning 4 Weeks", "One-Time",
] as const;
...
function sanitizeTenantKey(value: unknown): string {
  if (typeof value !== "string") return "";
  return value.trim().slice(0, 128);
}

function validateTenantKey(value: string): boolean {
...
function validateServiceType(value: string | undefined): boolean {
  return !value || SERVICE_TYPES.includes(value as typeof SERVICE_TYPES[number]);
}

function validateFrequency(value: string | undefined): boolean {
  return !value || FREQUENCY_OPTIONS.includes(value as typeof FREQUENCY_OPTIONS[number]);
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const tenantKey = sanitizeTenantKey(formData.tenant_key || req.headers.get("x-cleanflow-tenant"));
    if (tenantKey && !validateTenantKey(tenantKey)) {
      return new Response(JSON.stringify({ error: "Invalid tenant key", field: "tenant_key" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!formData.turnstile_token) {
      return new Response(JSON.stringify({ error: "Bot verification is required", field: "turnstile_token" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const ipDigest = await crypto.subtle.digest(
      "SHA-256",
      new TextEncoder().encode(`${rateLimitSalt}:${tenantKey || "single-company"}:${clientIp}`),
    );
    ...
    const turnstileResponse = await fetch("https://challenges.cloudflare.com/turnstile/v0/siteverify", {
      ...
      body: new URLSearchParams({ secret: turnstileSecret, response: formData.turnstile_token, remoteip: clientIp }),
    });
    const turnstileResult = await turnstileResponse.json();
    if (!turnstileResponse.ok || turnstileResult.success !== true) {
      return new Response(JSON.stringify({ error: "Bot verification failed", field: "turnstile_token" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!name || name.length < 2) {
      return new Response(JSON.stringify({ error: "Valid name is required", field: "name" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!phone && !email) {
      return new Response(JSON.stringify({ error: "Phone or email is required", field: "contact" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (phone && !validatePhone(phone)) {
      return new Response(JSON.stringify({ error: "Invalid phone format", field: "phone" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (!validateServiceType(formData.service_type) || !validateFrequency(formData.frequency)) {
      return new Response(JSON.stringify({ error: "Invalid service selection", field: "service_type" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (formData.preferred_date && !/^\d{4}-\d{2}-\d{2}$/.test(formData.preferred_date)) {
      return new Response(JSON.stringify({ error: "Invalid preferred date", field: "preferred_date" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (invalidRoomCount || invalidSquareFeet || (formData.has_pets != null && typeof formData.has_pets !== "boolean")) {
      return new Response(JSON.stringify({ error: "Invalid property measurements", field: "property" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(JSON.stringify({ success: true, message: "Thank you! We'll be in touch soon." }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error: unknown) {
    console.error("Error in website-lead-capture:", error);
    return new Response(JSON.stringify({ error: "Unable to process the lead request" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
