...
-- This migration intentionally fails instead of guessing ownership when legacy
-- data is ambiguous across multiple companies.
...
BEGIN
  ...
  ELSIF company_count > 1 THEN
    -- Recover ownership from already-related rows where the parent is scoped.
...
-- Browser writes inherit tenant from the authenticated staff identity.
...
-- Once ownership is known, make the invariant explicit.
DO $$
DECLARE table_name text;
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'staff', 'customers', 'leads', 'jobs', 'invoices', 'transactions',
    'payroll_records', 'payroll_rules', 'conversations', 'automation_configs',
    'automation_logs', 'broadcast_messages', 'transaction_rules'
  ] LOOP
    IF NOT EXISTS (EXECUTE format('SELECT 1 FROM public.%I WHERE company_id IS NULL LIMIT 1', table_name)) THEN
      NULL;
    END IF;
  END LOOP;
END
...
-- Tenant-aware helpers used by policies below.
...
-- Replace broad role-only policies on directly scoped tables.
DO $$
DECLARE table_name text;
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
...
-- Field access must also remain inside the current company.
...
-- Support is tenant-scoped and owner-scoped for regular users.
...
-- Bootstrap now establishes the staff-to-company membership at creation time.
